...
$(document).ready(function() {
    ...
    userProfileLink.click(function(){
        ...
        $('#card-user-name').text( $(this).attr("data-name") + ' ' + $(this).attr("data-surname") );
        //$('#card-user-name').text( $(this).attr("data-name") + ' ' + $(this).attr("data-surname") + age );
        $('#card-user-jobsearch').text( $(this).attr("data-jobsearch") );
        $('#card-user-education').text( $(this).attr("data-education") );
        $('#card-user-experience').text( $(this).attr("data-experience") );
    ...
    acceptInvitationButton.click(function(){
        $('#accept-invitation-team-id').val(this.dataset.team)
        $('#accept-invitation-id').val(this.dataset.id)
        openModal('#accept-invitation');
    })

    const declineInvitationButton = $('.decline-invitation');
    declineInvitationButton.click(function(){
        $('#decline-invitation-team-id').val(this.dataset.team)
        $('#decline-invitation-id').val(this.dataset.id)
    ...
    if (createTeamForm) {
        ...
        createTeamForm.addEventListener("change", function (event) {
            ...
            if (target.name && target.name == "team") {
    ...
    function search(searhValue) {
        if (searhValue.length >= 3){
            let team;
            $('.teams-list .teams-item').each(function(i, elem) {
    ...
    if (searchTeam) {
        searchTeam.value = '';
        searchTeam.addEventListener('keyup', function (event) {
            const value = this.value;
            search(value.trim());
        })
    }

    const searchTarget = document.getElementById('search-product');

    if (searchTarget) {
        searchTarget.addEventListener('paste', (event) => {
    ...
    if (tasksFilter) {
        $(tasksFilter).on('change', function (event) {
            ...
            if ( $(tasksFilter).val() == '#'){
                ...
                $('.teams-list .teams-item').each(function(i, elem) {
                    ...
                    if ($(this).attr("data-id") == ''){
                        ...
                        if ($(tasksFilter).val() == $(this).attr("data-id") ) {
                            $(this).show();
                        } else {
                            $(this).hide();
                        }
