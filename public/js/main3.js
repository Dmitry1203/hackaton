...
// advanced
function addAdvancedPreview(input, file) {
    if (!input || !file) return;

    const previewContainer = input.closest('.custom-upload').querySelector('.preview-container');
...
function removePreview(button) {
    const input = button.closest('.custom-upload').querySelector('input');
    if (!input) return;
...
function sendInfoForm(event, list /*, dropzone*/) {
    let send = true;
...
// html
function getCriteriaItemHTML(length) {
    return `
...
function initTimeInput(input) {
    ...
    timeInput.dates.parseInput = (value) => { return new tempusDominus.DateTime(moment(value, "HH-mm")) };
}
...
document.addEventListener("DOMContentLoaded", event => {
    ...
    document.addEventListener('input', (event) => {
        ...
        if (target.closest('.custom-upload-simple')) {
            ...
            addAdvancedPreview(target, file);
        }
    });
    ...
    if (infoForm) {
        ...
        var myDropzone = new Dropzone(document.querySelector('.custom-upload-multi'), {
            url: "/",
            paramName: "partner__logo",
            thumbnailWidth: null,
            thumbnailHeight: null,
            parallelUploads: 20,
            previewTemplate: previewTemplate,
            autoQueue: false,
            previewsContainer: ".custom-upload-multi .preview-container",
            clickable: ".custom-upload-multi .add-button",
        ...
        */
        ...
        infoForm.addEventListener('submit', function (event) {
            ...
            sendInfoForm(event, inputList /*myDropzone*/);
        });
        ...
        if (criteriaContainer && addCriteriaButton) {
            addCriteriaButton.addEventListener('click', function(event) {
                event.preventDefault();
                const length = criteriaContainer.querySelectorAll('.criteria-item').length;
                const criteriaItemHTML = getCriteriaItemHTML(length);
...
$(document).ready(function() {
    ...
    saveSolution.click(function(e) {
        ...
        //передали содержимое для отправки
